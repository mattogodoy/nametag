...
- On nametag.one, an ntfy or webhook destination must be HTTPS, and cannot point at a private or internal address. A self-hosted instance has no such restriction, since reaching a server on your own network, such as a LAN ntfy box, is exactly the point.
