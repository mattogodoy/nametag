...
If no trusted client IP can be determined at all, requests that carry no other identifier fall into one shared bucket. That bucket is per process rather than per deployment, and it allows considerably more than the per-IP limit for the same endpoint, so a single caller cannot deny login or password reset to everyone on the instance. Nametag logs a JSON line with the event `rate_limit.no_trusted_client_ip` when this happens, repeated every 15 minutes with a count of affected requests, so it is worth alerting on: it means rate limiting is running in a degraded mode and `TRUSTED_PROXY_COUNT` or `TRUSTED_PROXY_HEADER` probably needs attention.
